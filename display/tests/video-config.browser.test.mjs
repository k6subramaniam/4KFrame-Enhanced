...
test('config events synchronize active video playback and surface restart rejection', { timeout: 30_000 }, async (t) => {
  if (!browser) {
  ...
  try {
    await waitForServer(`${origin}/tests/video-config.browser.html`);
    const result = await run(browser, [
      ...
      '--dump-dom',
      `${origin}/tests/video-config.browser.html`,
    ]);
