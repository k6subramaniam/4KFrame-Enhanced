...
export function filterLibraryItems(source, filters) {
    ...
    return source.filter((item) => {
        ...
        if (Number.isFinite(start) && date < start)
            return false;
        if (Number.isFinite(end) && date > end)
            return false;
        if (filters.kind !== 'all' && item.kind !== filters.kind)
            return false;
        if (filters.uploader !== 'all' && itemUploader(item) !== filters.uploader)
            return false;
        if (filters.storage !== 'all' && itemStorage(item) !== filters.storage)
            return false;
        if (!matchesSize(item.sizeBytes, filters.size))
            return false;
        return true;
    });
}
function matchesSize(bytes, filter) {
    if (filter === 'all')
        return true;
    if (filter === 'unknown')
        return bytes === undefined || !Number.isFinite(bytes);
    if (bytes === undefined || !Number.isFinite(bytes))
        return false;
    if (filter === 'under-10mb')
        return bytes < 10 * MB;
    if (filter === '10-100mb')
        return bytes >= 10 * MB && bytes < 100 * MB;
    if (filter === '100mb-1gb')
        return bytes >= 100 * MB && bytes < GB;
    return bytes >= GB;
}
...
//# sourceMappingURL=mediaFilters.js.map